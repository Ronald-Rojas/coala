...
Inside ``@linter`` there are only a few output formats which are supported. To
specify an own output parsing or processing behaviour, simply override
``process_output``.
...
- ``process_output_corrected``: Extracts results (with patches) using a
